...
**Текущая фаза:** Create. Шаги 1, 2 и 4 дорожной карты сделаны (ядро заявки; дома, роли, REST, уведомления; мини-приложение), стенд и мини-приложение — `https://domovoy.prooood.ru` (Yandex Cloud, автодеплой из `main`); план и следующий шаг — [docs/STATUS.md](docs/STATUS.md). LLM — только хостинговая по API, без дообучения ([D-007](docs/DECISIONS.md)).
...
| `ml/` | отдельный сервис классификации жалоб (категория + аварийность), свой `pyproject.toml`/Docker/`uv`, порт 8100; каркас LoRA от 22.09 удалён и не связан с этим кодом ([DECISIONS.md D-005, D-006](docs/DECISIONS.md)) | структура и HTTP-контракт — [ml/README.md](ml/README.md), [docs/CONTRACTS.md §4](docs/CONTRACTS.md); обученные модели (`*.cbm`, ~10 МБ) и `metrics.json` коммитятся в `ml/models/`; переобучение — `ml/training/train.py` |
| `infra/yc/`, `.github/workflows/deploy.yml` | прод на ВМ Yandex Cloud: push в `main` → тесты → образы в Container Registry → `infra/yc/deploy.sh` на ВМ | [D-009](docs/DECISIONS.md); секреты только на ВМ (`/opt/domovoy/.env`, `/opt/domovoy/backend/.env`) |
| `infra/fly/` | конфиги старого тестового деплоя на fly.io (`domovoy-test`, `domovoy-test-db`), машины остановлены 27.09 | не использовать: вебхук теперь на Yandex Cloud |
