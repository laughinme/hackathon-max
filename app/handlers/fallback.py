...
from app.categories import guess_category
from app.handlers.create import CATEGORY, TURNS, _analyze_and_render
...
async def on_free_text(
    ...
    if len(text) < MIN_PROBLEM_LENGTH:
        await render(
            event, context, texts.unknown_message(), keyboards.main_menu()
        )
        return
...
async def on_error(event: ErrorEvent) -> None:
    ...
    logger.exception(
        "Ошибка обработки события %s", type(event.update).__name__,
    ...
    try:
        await render(
            update, context, texts.error_occurred(), keyboards.main_menu()
        )
