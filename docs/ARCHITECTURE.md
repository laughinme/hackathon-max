...
## 7. Текущее состояние кода (24.09, после шагов 1–5)

Сделано ([D-008](DECISIONS.md), [D-010](DECISIONS.md)):

| Слой | Что есть |
|---|---|
| `domain/` | `tickets/` (агрегат, переходы по ролям, SLA, календарь, ответственность и навигатор, «Я тоже»), `housing/` (УО, дом, житель, диспетчер), `chats/` (домовой чат, подсказка, признаки жалобы), `notifications/` |
...
| `bot/` | хендлеры `start`, `create`, `my_requests`, `dispatcher`, `privacy`, `house`, `group` (домовой чат), `fallback`; фильтры `scopes.py` (личка / группа); `views.py`, тексты и клавиатуры по ролям, `notifications.py` (отправка из outbox), `errors.py` |
...
| `frontend/` | мини-приложение: экраны жителя (дом, мои заявки, заявка с подтверждением) и диспетчера (очередь со счётчиками и фильтрами, смена статуса); стек экранов вместо роутера ([D-011](DECISIONS.md)) |
