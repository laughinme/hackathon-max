...
def load_config() -> Config:
    ...
    token = (os.getenv("MAX_TOKEN") or os.getenv("MAX_BOT_TOKEN") or "").strip()
    if not token:
        raise RuntimeError(
            "Не задан MAX_TOKEN. Скопируйте .env.example в .env "
            "и укажите токен бота, выданный организаторами хакатона."
        )
